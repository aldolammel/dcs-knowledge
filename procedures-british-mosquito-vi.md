# Procedures > British Mosquito FB Mk VI

---

**Controls:**
- [controls-british-mosquito-vi](controls-british-mosquito-vi.md)
- IL2 GB, controls, Mosquito (soon)
**Guns & Fuel:**
- [guns-and-fuel-british-mosquito-vi](guns-and-fuel-british-mosquito-vi.md)
**Basic Gameplay:**
- [basic-about-aircrafts](basic-about-aircrafts.md)
- This plane's official doc: [https://www.digitalcombatsimulator.com/en/downloads/documentation/dcs-mosquito_early_access_manual_en/](https://www.digitalcombatsimulator.com/en/downloads/documentation/dcs-mosquito_early_access_manual_en/)

```
Introduced in 1942, this is a variant of the British 'Wooden Wonder', the twin-engine Mosquito fighter-bomber. Renowned as one of the fastest propeller-driven aircraft of WWII, it features potent centralized cannon firepower and flexible internal and wing-mounted payloads. It's a night interceptor too, using the Hit-and-Run doctrine.
```
## Pros:

- Light (wooden-made);
- Perfect for fast and low-altitude missions at night;
- Designed for Anti-ship and ground attack missions;
- [basic-about-maneuvers](basic-about-maneuvers.md)- Fire and leave, never orbiting a target to re-engage.
## Cons:

- No armor at all;
- Avoid dogfights;
- Avoid turns and aggressive dives;
- [basic-about-maneuvers](basic-about-maneuvers.md)- avoid negative G-forces (engines will fail) and aggressive dives (wings will break);

---
## **1) Mosquito > Cold start:**

1. <span style="color: grey;">(Maybe)</span> Turn your `personal flashlight` ON;
2. Open `left window` (crew needs to listen to you);
3. (Maybe) Looking to `Radio Operator` seat, lower the `Armored Backrest`;
4. To `ATC`, request <span style="color: orange;">start-up</span> procedure;
5. In `Briefing` screen, check <span style="color: orange;">weather temp.</span>;
6. (Maybe) To `Ground Crew`, request <span style="color: orange;">rearm and refuel</span>, and wait for the answer;
7. **Electric:**
	1. To `Ground Crew`, request <span style="color: orange;">ground electrical power ON</span>;
	2. (Maybe) `Cockpit lights` should be <span style="color: orange;">ON</span>;
	3. Unblock the `Electric Power Master` switch (front panel, left side of those two red bts) by pulling only the metal protection UP;
	4. In `Radiators` panel (front blue panel, above those two red bts):
		1. Make sure both `Engine Water Cooling Flaps` switches are OFF (up);
		2. Turn `Air Filter` (of carburetor) switch ON (down);
    5. (Maybe) For Bombing mission:
	    1. Make sure all 6 `Bomb panel` switches (right knee level) are OFF (up);
	    2. ~~If cold/high-alt.: Turn `Anti-Icing System' (right foot) to ON~~ ==(not working yet)==;
    6. In `Circuit Breakers` panel (right side, upper white buttons):
        1. Mandatory:
            - Keep `Fuel Pump` switch OFF (up). Main engine-driven pumps feed directly from the wing/inner tanks even when `External Fuel` tanks are in use.
            - Turn `Nav. Lights` switch ON (down);
        2. (Maybe) Cold/Rain weather:
            - If cold: `Pitot Heat` switch ON (down);
            - If rain: `Pitot Heat` switch ON (down);
8. **Fuel:**
    1. Behind `Pilot Seat` (use the radio operator seat):
        - In `Port Fuel Tank` selector, set it to `Outer Tanks` pos (right);
        - In `Starboard Fuel Tank` selector, set it to Outer Tanks pos (left);
        - Set `Fuel Tank Pressure` valve (silver one above both red bts) to ON (smaller side down);
        - Make sure `Fuel Transfer` valve (other silver one aside) is OFF (vertical);
    2. Slide both `Throttle` levers forward just 10% to make the fuel flow soon;
    3. ==(Maybe) Using an `External Fuel Tank` :==
        1. ==Pull `Drop Tank` lever (right leg) to ON (back);==
        2. ==Turn `Main Tank` lever to OFF (down);==
9. **Engine:**
    1. Both `Pitch` levers (side by side with `Throttle`), slide them forward until the end;
    2. Make sure `Supercharger Gear` switch (rear of the `Throttle`) is in MOD pos (down);
    3. Ask `Ground Crew` :
        - To place `Wheel Chocks` (to avoid the plane moving);
        - To `start priming engines`;
    4. In `Magnetos` switches (those 4 switches in `Electric Power Master` switch):
        1. Left engine:
            1. Turn both left `Magnetos` ON (up);
            2. Open `Starter` and `Booster Coil button` covers (above those two big red bts);
            3. Press and hold both buttons simultaneously until the propeller starts to spin;
            4. Info: <span style="color: orange;">one of the engines should starts now!</span>
                1. (Maybe) If it failed, ==xxxxxxxxxxxxxx== and restart the procedure, starting with the "2 buttons" step and so on!
            5. Using the `Left Throttle` only, making your left engine `Tachometer` (front panel, lower left side) park at 12 (1200 RPM);
        2. Right engine:
            1. Repeat the same logic for the right engine;
        3. Check `Oil Pressure` gauge (front panel, left orange square monitors), both should be within 60-120ft/in² after a little while;
10. **Warm the engine up:**
    1. **Meanwhile:**
        - (Maybe) Ask the crew to disconnect the `Ground Electric Power`;
        - ==Turn Radio (xxxxxxxx) ON and set this up;==
        - ==In `Altimeter` gauge, set it to zero!==
        - In `Compass Course Ring` (front of `Throttle`), align the North with the North Magnetic (an orange cross);
        - In `Directional Gyro` gauge (front panel, above the stick head pos), set it to the same value as the `North Magnetic` shows with a `bold stripe` pointed to the bird's nose direction;
        - (Maybe) In `Circuit Breakers` panel, turn `UV Light` switch ON (down), and adjust the cockpit lights intensity based in your taste;
    2. Wait for `Oil temp.` gauges (front, left side, between `Oil Pressure` gauges) to reach 40ºC / Wait for `Radiator temp.` gauges (below the `Oil temp.` gauges) to reach 60ºC (~3min);
    3. Once warmed up, in Radiators panel, turn both `Engine Water Cooling Flaps` switches ON (down);
    4. Slide back `Throttles` to idle;
    5. Ask `Ground Crew` to remove the `Wheel Chocks`;
    6. In `Stick` , press the `Brake Handle` to release the wheels;
    7. Close any `open windows`;
11. (Maybe) Night/Low visibility, turn both `Landing Light` (right foot) knobs ON (right);
12. Info: ready to go!

---
## **2) Mosquito > Taxiing:**

1. Make sure `Nav. Lights` are ON;
2. (Maybe) Case `External Fuel Tanks`, make sure ==selector position xxxxxxxxxxxx;==
3. (Maybe) Make sure `Landing Light` knobs are ON;
4. (Maybe) Looking to `Radio Operator` seat, lower the `Armored Backrest`;
5. Make sure all windows are closed;
6. Make sure `Pitch` levers are fully forward;
7. In `Radiators` panel:
    1. Make sure both `Engine Water Cooling Flaps` switches are ON (down);
    2. Make sure `Air Filter` (of carburetor) is ON (down);
8. Say on radio: `YourCallsign` taxiing!
9. To move the bird, use only `Throttle` at 11.5 (1150 RPM), `Rudder` , and `Wheel Brakes` gently. The brake system gets the rudder direction to apply the right pressure to the wheels during a turn;
10. (Maybe) To `ATC`, request <span style="color: orange;">takeoff</span> procedure and wait for the answer;

---
## 3) Mosquito > Takeoff:
EXTRA TIP: Check where you are on the map (or on the kneeboard);

1. **Preparing:**
    - Make sure `Nav. Lights` are ON;
    - (Maybe) Case `External Fuel Tanks`, make sure ==selector position xxxxxxxxxxxx;==
    - (Maybe) Make sure `Landing Light` knobs are ON;
    - (Maybe) Looking to `Radio Operator` seat, lower the `Armored Backrest`;
    - Make sure all windows are closed;
    - Make sure `Pitch` levers are fully forward;
    - Make sure both `Fuel Tank` selectors are in the `Outer Tanks` positions;
    - Unblock `Gear` lever (right knee, red one), making sure the `protection plate` is felt to pilot direction;
    1. In `Radiators` panel:
        - Make sure both `Engine Water Cooling Flaps` switches are ON (down);
        - Make sure `Air Filter` (of carburetor) is ON (down);
    - (Maybe) Heavy duty mission:
        - Turn `Supercharger` (rear of `Throttle`) to `AUTO` (up);
    - Trim:
        - `Trim Elevator` (monitor above `Throttle`):
            - If no bombs: to `1` pos (nose down);
            - If with bombs: to `2` pos (nose down);
        - `Trim Rudder` (monitor right side of `Reflector Sight`) to right until the "Trim" pos;
        - `Trim Ailerons` (right knee) to neutral (center) pos;
    - Flaps:
        - If only guns and full internal fuel tank or less: no `flaps` needed;
        - If heavier (bombs and/or loaded w/ `External Fuel Tank`), unblock `Flaps` lever (front panel, `F` lever), and adjust it to 15º (monitor on the lever's left side);
    - Make sure everything from "MEANWHILE" Cold-start procedures was done; 
2. **Go:**
    1. Say on radio: `YourCallsign` rolling!
    2. Holding `brake pedals`, keep `Stick` idle, and slide `Throttle` forward up to 2.000 RPM;
    3. Release `brake pedals` and speed up to 3.000 RPM;
    4. When 110mhp, gently pull the `Stick` back to fly;
3. **Once airborne:**
    - Press `Brakes` to stop the tires;
    - `Gear up`, holding the red lever up for 5 secs - limit speed's ==XXXmph==;
    - (Maybe) `Flaps up` to 0º - limit speed's ==XX0mph==;
    - Say on radio: `YourCallsign` airborne!
    - Turn `Air Filter` (of carburetor) is OFF (up);
    - (Maybe) Turn `Supercharger to` ON/AUTO (up);
    - Trim adjustments!
    1. **(Maybe) In combat skies:**
        - Turn `Nav. Lights` OFF;
        - Turn `Cabin illumination OFF`;
        - Turn `Reflector Sight` switch (`Circuit Breakers` panel) ON (down);
        - Unblock and turn `Gun Firing Master` switch (right knee, green button) to FIRE (down) pos;

---
## **4) Mosquito > Climbing & Cruise:**

- **Climbing:**
    - Ceiling: 35.000ft (~10.600m);
    - `Pitch` 2850 RPM, `Throttle` +9 Boost (==nose up to 170mph stable==);
    - From 10.000ft (~3.000m):
	    - In `Oxygen` valve (behind the `Stick`, small wing knob), open it (vertical);
    - ==From 18.000ft alt:==
        - ==Make sure `Circuit Breaker Pitot Heating` switch is ON (down);==
        - ==Turn `Windscreen De-Icer` knob (right leg) to ON (upper red end pointing to the back);==
    - ==From 25.000ft alt:==
        - ==Set `Fuel Tank Pressure` valve to ON (right);==
- **Cruise:**
    - (Maybe) `External Fuel Tanks` management:
        - Once `Inner/Wing Tanks` use ~20% of their fuel, turn `Fuel Pump` switch (right panel, white buttons) ON (down) to transfer fuel from External tanks to the inner ones;
    - ==Safe cruise MAX = `Pitch` 2650 RPM, `Throttle` +7 Boost (Fuel = 80 igal/h);==
    - ==To save fuel = `Pitch` 1800 RPM, `Throttle +2` Boost (Fuel = 30 igal/h);==
    - ==Make sure `Radiators` switch is Auto (up);==
- **Communication:**
	- [basic-about-communication](basic-about-communication.md)

---
## **5) Mosquito > Combat:**

- **General:**
    - [basic-about-maneuvers](basic-about-maneuvers.md)
    - (Maybe) `External Fuel Tanks` should be <span style="color: orange;">jettison</span>:
	    - Read section: 6) Discarding `External Fuel Tanks`;
    - ==Never use high `Boost` (`Throttle`) settings with low RPM (`Pitch`);==
        - ==For less power = First, reduce `Boost` , and then RPM;==
        - ==For more power = First, increase RPM, and then `Boost`;==
    - ==Stall speed < 160mph (257Km);==
    - ==Dive speed limit: 450mph alt 0-20K feet;==
    - ==After 25K feet, set `Fuel Tank Pressure` valve to ON (right);==
- **Dogfight:**
    - ==xxxx==
- **Intercepting:**
    - ==xxxx==
    - ==For Rockets use, check `Special Ops` section;==
- **Ground attack:**
    - Bombing:
	    - In Bomb panel (right side of `Flaps` lever):
		    - Activate `Bomb Panel` dual-switch (down);
		    - Activate every bomb you wanna use (down):
			    - Upper dual switches = wing ones;
			    - Middle dual switches = Internal/fuselage ones;
		    - Activate `Fuse` lower switches (down);
	    - (Maybe) Push `Bomb Bay` lever ("B" one) to open it, and wait for the Yellow light signal;
	    - **Critical:** long trigger press to release all of them; or single press for just one;
	    - Straight approach:
		    - Vel. 160mph, ==Alt. 6000ft;==
	    - Dive approach:
		    - ==[https://www.youtube.com/watch?v=\_C7n0GJH_DU](https://www.youtube.com/watch?v=_C7n0GJH_DU)
		    - ==Vel. 230mph, Alt. 6000ft;==
		    - ==Dive 45-60º, release bomb(s) at 3000ft, recover alt from 2500ft max;==
	    - (Maybe) In `Bomb Bay` lever, pull it up to close it;﻿
    - Strafing:
        - ==xxxx==

---
## 6) **Mosquito >** Special Ops:

- **(Maybe) Firing rockets:**
    1. Turn `Rockets Master` switch (side by side with `Supercharger` switch) ON (down);
    2. ~~In `Rockets Manual Advancement` silver button, select how many rockets to use simultaneously~~ (not working);
    3. In `Rockets Salvo` switch (front panel, upper left side), define if you want (ON/down) or not (OFF/up) to fire all rockets at the same time;
- **(Maybe) Discarding `External Fuel` Tank:**
    1. Turn `Fuel Pump` switch (right panel, white buttons) OFF (up);
    2. ==Push `Drop Tank` lever (right leg) to OFF (forward);==
    3. ==Pull `Drop Tank Release` valve (red one with the `Drop Tank` lever);==

---
## 7) **Mosquito >** Failure Actions:

- **For Fuel leaking:**
    1. ==Pull `Mixture Control Select` lever full back to stop fuel flow;==
    2. ==Turn `Circuit Breaker Fuel Pump` OFF (up);==
    3. ==(Maybe) Keep the `External Fuel Tank` until you figure out where the leak is;==
        1. ==If fueling from `External Fuel Tank` , return to the Main Fuel Tank and jettison the external tank;==
        2. ==Open `Mixture Control Select` lever again;==
        3. ==Turn `Circuit Breaker Fuel Pump` ON (down);==
        4. ==Check the `Fuel` gauge. If not leaking anymore, restart the engine!==
- **For Fuel Low Pressure:**
    1. ==Set `Fuel Tank Pressure` to ON (right);==
- **For Engine on fire:**
    1. ==Pull `Mixture Control Select` lever full back to stop fuel flow;==
    2. ==Set `Supercharger Mode Auto` switch to `M.S.` (up);==
    3. ==Turn `Circuit Breaker Fuel Pump` OFF (up);==
    4. ==In `Radiators Flaps Auto` switch, set to ON (down);==
- **For Electrical issues:**
    1. ==In `Radiators Flaps Auto` switch, set to ON (down);==
- **For Radiators damage:**
    - ==In `Radiators Flaps Auto` switch, set to AUTO/OFF (up);==

---
## **8) Mosquito > Landing:**

1. **Before the motherland coast:**
    1. (Maybe) Night/Low visibility:
        - Turn `Resin Air Lamps` switch (right panel, left of `Morse Code` panel) to ON (down);
        - Turn `Type F Transmitter` light (an IR tail light) switch (right panel, under a golden valve) to ON (right/back);
2. **After that, already at home sky:**
    1. (Maybe) To `ATC`, request <span style="color: orange;">landing</span> procedure and wait for the answer;
    2. (Maybe) Release `External Fuel Tank`;
    3. `Nav. Lights` ON;
    4. Turn `Supercharger Gear` switch to MOD (down);
    5. Make sure `Pitch` lever is in fully forward pos;
    6. `Trim_:
        - `Trim Elevator` (monitor above `Throttle`):
            - ==If no bombs: to 1 pos (nose down);==
            - ==If with bombs: to 2 pos (nose down);==
        - `Trim Rudder` (monitor right side of `Reflector Sight`) ==to right until the "Trim" pos;==
        - `Trim Ailerons` (right knee) ==to neutral (center) pos;==
    7. Unblock `Flaps` lever, and adjust it to ==15º== (monitor on the lever's left side);
    8. ==Vel. 160mph, alt. 1000 feet;==
    9. (Maybe) Emergency landing:
        1. ==Jettison the `Canopy` with its red lever (over your head);==
        2. ==Set `Supercharger Mode Auto` switch to `M.S.` (up);==
        3. ==Turn `Circuit Breaker Fuel Pump` OFF (up);==
        4. ==(Maybe) Don't use Gear Down for soft terrain/water;==
    10. ==In `Stick` , press `Safety Guns Pin` IN (on)!==
    11. ==In `Carburetor Air Control` lever (left leg), set it to `Filter In` (up);==
3. **Approaching/Landing:**
    1. (Maybe) Night/Low visibility, turn both `Landing Light` (right foot) knobs ON (right);
    2. `Flaps` down - limit speed's 140mph;
    3. `Gears` down - limit speed's 160mph;
    4. Vel. 110-100mph (RPM ~XX);
    5. Alt. 650-350 feet;
    6. (Maybe) Emergency landing:
        - Set `Magneto` switches OFF (down);
        - In `Main Tank` lever, set to OFF (down);
        - Pull `Mixture Control Select` lever full back;
    7. Once on the ground (70-65mph), `Throttle` in idle, and use `Wheel Brakes` carefully;
4. **After regular landing:**
    1. Leave any runway;
    2. Open the `left window` (crew needs to listen to you);
    3. `Flaps` up;
    4. (Maybe) Turn `Circuit Breaker Pitot Heating` switch OFF (up);
    5. (Maybe) Turn `Resin Air Lamps` switch to OFF (up);
    6. (Maybe) Turn `Type F Transmitter` light switch to OFF (left/forward);
    7. Taxi it;

---
## **9) Mosquito > Turn it off:**

1. **Weaponry:**
    - Turn `Gun Firing Master` switch (right knee) to SAFE (up), and block it;
2. `Left window` should be <span style="color: orange;">open</span>;
3. To `Ground Crew`, request to place <span style="color: orange;">wheel chocks</span>;
- (Maybe) Turn `Landing Light` knobs OFF (left);
1. **Engine:**
    1. ==Cool the engine for 2min at 800 RPM;==
        1. ==**Meanwhile:**==
            1. ==Make sure `Safety Guns Pin` is IN (on);==
            2. ==Turn `Oxygen` knob OFF (horizontal pos);==
    2. ==Pull the `Pitch` lever full back;==
    3. ==Once the blades have stopped completely, turn `Magnetos` OFF;==
2. **Fuel:**
    1. ==Turn `Fuel Tank Pressure` valve OFF (left);==
    2. ==Pull `Mixture Control Select` lever full back;==
    3. ==In `Main Tank` lever, set it OFF (down);==
3. **Electric:**
    1. In `Radiators` panel, turn OFF (up) all switches;
    2. In `Circuit Breakers` , turn OFF (up) all switches;
4. Done!