...
- This plane's official doc: [https://www.digitalcombatsimulator.com/en/downloads/documentation/dcs-thunderbolt_flight_manual_en/](https://www.digitalcombatsimulator.com/en/downloads/documentation/dcs-thunderbolt_flight_manual_en/)
...
1. <span style="color: grey;">(Maybe)</span> Turn your `personal flashlight` <span style="color: orange;">ON</span>;
2. `Canopy` must be <span style="color: orange;">open</span> (crew needs to listen to you);
...
5. <span style="color: grey;">(Maybe)</span> To `Ground Crew`, request <span style="color: orange;">'Rearm and Refuel'</span>, and wait for the answer;
6. **Electric:**
    1. To `Ground Crew`, request <span style="color: orange;">'Ground Electrical Power' ON</span>;
    2. In `Circuit Breakers` panel (left knee, floor switch panel):
		1. <span style="color: grey;">(Maybe)</span> `Cockpit lights`:
...
9. **Warm the engine up:**
    ...
    3. **Meanwhile:**
        ...
        - In `Artificial Horizon`, <span style="color: orange;">unlock it</span>, using its big `Cage` knob, <span style="color: orange;">twisting it to ON</span> pos;
...
1. `Nav. Wing Lights` should be <span style="color: orange;">ON</span>;
2. `Nav. Tail Light` should be <span style="color: orange;">ON</span>;
3. <span style="color: grey;">(Maybe)</span> `Landing Light` switch (right side of `Nav. Tail Light`) should be <span style="color: orange;">ON (up)</span>;
...
1. **Preparing:**
    - `Nav. Wing Lights` should be <span style="color: orange;">ON</span>;
    - `Nav. Tail Light` should be <span style="color: orange;">ON</span>;
    - <span style="color: grey;">(Maybe)</span> `Landing Light` switch <span style="color: orange;">OFF (middle)</span>;
    - <span style="color: grey;">(Maybe)</span> Case `External Fuel Tanks`, make sure `External Tanks Fuel Selector valve` is correct;
...
2. **Go:**
    ...
    2. With `Stick` <span style="color: orange;">idle</span>, slide `Throttle` <span style="color: orange;">fully forward</span> slowly;
        - **If normal weight:**
            - When the speed hits <span style="color: orange;">100mph</span>, gently move `Stick` forward;
            - At <span style="color: orange;">110 knots</span>, pull `Stick` back to fly;
            - Once <span style="color: orange;">150mph</span>, start to climbing;
        1. **If heavy weight:**
            - ==xxxxxxxxxxxxxxxx==
            - ==xxxxxxxxxxxxx==
3. **Once airborne:**
...
- **For Fuel leaking:**
    ...
    3. ==(Maybe) Keep the `External Fuel Tank` until you figure out where the leak is;==
        ...
        4. ==Check the `Fuel` gauge. If not leaking anymore, restart the engine!==
- **For Fuel Low Pressure:**
    1. ==Set `Fuel Tank Pressure` to ON (right);==
- **For Engine on fire:**
    1. ==Pull `Mixture Control Select` lever full back to stop fuel flow;==
    2. ==Set `Supercharger Mode Auto` switch to `M.S.` (up);==
    3. ==Turn `Circuit Breaker Fuel Pump` OFF (up);==
    4. ==In `Radiators Flaps Auto` switch, set to ON (down);==
- **For Electrical issues:**
    1. ==In `Radiators Flaps Auto` switch, set to ON (down);==
- **For Radiators damage:**
    1. ==In `Radiators Flaps Auto` switch, set to AUTO/OFF (up);==

---
## **8) P47 > Landing:**

1. **Before:**
    1. <span style="color: grey;">(Maybe)</span> To `ATC`, request <span style="color: orange;">landing</span> procedure and wait for the answer;
    2. <span style="color: grey;">(Maybe)</span> Release `External Fuel Tank`;
    ...
    11. ==In `Stick`, press `Safety Guns Pin` IN (on)!==
    12.  In`Air Filter Control` lever (left side, on floor), active it, pulling it UP;
2. **Approaching/Landing:**
    1. ==`Flaps` down - limit speed's 140mph;==
    2. ==`Gears` down - limit speed's 160mph;==
    3. ==Vel. 110-100mph (RPM ~XX);==
    4. ==Alt. 650-350 feet;==
    5. <span style="color: grey;">(Maybe)</span> Emergency landing:
        - ==Set `Magneto` switches OFF (down);==
        - ==In `Main Tank` lever, set to OFF (down);==
        - ==Pull `Mixture Control Select` lever full back;==
    6. ==Once on the ground (70-65mph), `Throttle` in idle, and use `Wheel Brakes` carefully;==
    7. ==When `Tail Wheel` touches the ground, pull the `Stick` back to lock it;==
3. **After regular landing:**
    1. ==Release the `Stick` to unlock the `Tail Wheel`;==
    2. Leave any runway;
    3. `Flaps` up;
    4. Turn `Circuit Breaker Pitot Heating` switch OFF (up);
    5. Taxi it;
...
5. **Electric:**
    ...
    2. `Nav. Wing Lights` <span style="color: orange;">OFF (middle)</span>;
    3. `Nav. Tail Light` <span style="color: orange;">OFF (middle)</span>;
    4. `Landing Light` <span style="color: orange;">OFF (middle)</span>;
    5. ==Turn OFF all `Circuit Breakers` (up)`, Gunsight, etc`;==
6. Done!